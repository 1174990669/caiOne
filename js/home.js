...
//				//console.log(ret);  //页面跳转显示
			if(ConfigObj.display){
	            if (ret.path == '/sporttery/jczq') this.gotoJczqBet();
	            else if (ret.path == '/numlottery/dlt') this.gotoDltBet();
	            else if (ret.path == '/numlottery/ssq') this.gotoSsqBet();
	            else if (ret.path == '/numlottery/jxk3') this.goFastK3Bet('jxk3');
	            else if (ret.path == '/numlottery/jlk3') this.goFastK3Bet('jlk3');
	            ...
	            else if (ret.path == '/numlottery/d3') this.gotoD3Bet();
	            else if (ret.path == '/numlottery/pl3') this.gotoPl3Bet();
	            else if (ret.path == '/numlottery/pl5') this.gotoPl5Bet();
	            else if (ret.path == '/sporttery/jc2x1') this.goto2x1Bet();
	            else if (ret.path == '/sporttery/spf14') this.goTotoBet();
	            else if (ret.path == '/sporttery/spf9') this.goR9Bet();
...
//        	  console.log(msg)
  ...
  homeObj.createNews = function(){
	  if(homeObj.ajaxData.header_news_top6){
		 ...
		 for(var i=0;i<data.length;i++){
				var itm = data[i];
				html += '<li><a href="javascript:void(0)"><span class="hot_1">热议</span>  '+ itm +'</a></li>'
			//}
		 }
		 // console.log(html)
...
//首页推荐比赛  homeObj_recomList
homeObj.getMatch = function(){
	var self = this;
	$.ajax({
	  	url : ConfigObj.localSite +  '?m=lottery.schedule.getRecommendSchedule',
									...
									'<div class="clearfix" id="homeObj_rec">'+
										'<div class="recomList_l fl">'+
											'<dl class="recomList_h" data-t="odds'+i+'" data-v="'+itm.mixOdds.FTBRQSPF[0]+'" data-k="3">'+
												...
												'</div>'+
												'<p class="recomList_odds pitch">'+itm.host_name_q+'&nbsp;<span id="win_odds">胜'+itm.mixOdds.FTBRQSPF[0]+'</span></p>'+
											'</dl>'+
										'</div>'+
										'<div class="recomList_m fl">'+
											'<dl class="recomList_h" data-t="odds'+i+'" data-v="'+itm.mixOdds.FTBRQSPF[1]+'" data-k="1">'+
												...
												'</div>'+
												'<p class="recomList_odds pitch" style="width:80%;"><span id="draw_odds">平'+itm.mixOdds.FTBRQSPF[1]+'</span></p>'+
											'</dl>'+
										'</div>'+
										'<div class="recomList_l fl">'+
											'<dl class="recomList_h" data-t="odds'+i+'" data-v="'+itm.mixOdds.FTBRQSPF[2]+'" data-k="0">'+
												...
												'</div>'+
												'<p class="recomList_odds pitch">'+itm.guest_name_q+'&nbsp;<span id="loss_odds">胜'+itm.mixOdds.FTBRQSPF[2]+'</span></p>'+
											'</dl>'+
										'</div>'+
									'</div>'+
									'<div class="home_r_buy clearfix">'+
										'<div class="home_re fl" style="width: 38%;"><p class="home_recom">预测奖金:<span id="recomList_bonus_'+i+'">0.00元</span></p></div>'+
