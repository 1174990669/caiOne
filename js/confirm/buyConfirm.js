...
//	  console.log('buyConfirm页面 setData ',obj);
  ...
  buyConfirmObj.setProjectData = function(){
   ...
   // console.log(this.MoneyObj.html(htmlA))
   console.log(data)
//	 console.log("支付初始化提交数据",data)
	 $.ajax({
        ...
        console.log(obj)
        // alert(1)
        // obj.info = $.parseJSON(Global.crypt(obj.info)); buyConfirm_MoneyObj
		  	var fo = $.parseJSON(Global.crypt(obj.info));
			// console.log('支付初始化返回数据：',fo);
   // console.log(ConfigObj.zdid)
        ...
        // console.log(pay_fre_zf)
      $.ajax({
        url:ConfigObj.localSite +'?m=cashier.deposit.number',
        data:secDat,
        dataType:'json',
        type:'post',
        success: function(obj){
          ...
          // alert(2)
              ...
              $.ajax({//次数接口
                url:ConfigObj.localSite +'?m=cashier.deposit.number_list',
                data:sDat,
                dataType:'json',
                type:'post',
                success: function(obj){
                 ...
                 /* for (var i = 0; i < obj.info.length; i++) {
                    // alert(4)
                    // android_obj.countFrequency(3)
                    // android_obj.calculateFrequency(5)

                    if (obj.info[i].type == 'pay_fre_zf') {
                       console.log(obj.info[i].num);
                      aadd.startPayCount = obj.info[i].num
                    }
                    if (obj.info[i].type == "pay_mon_zf") {
                      aadd.startPaySum = obj.info[i].pay_money
                    }
                    if (obj.info[i].type == "pay_mon_gc") {
                      aadd.startLotPaySum = obj.info[i].pay_money
                    }
                    if (obj.info[i].type == "pay_fre_gc") {
                      aadd.startLotPayCount = obj.info[i].num
                    }
                    if (obj.info[i].type == "rech_num") {
                      aadd.viewPayCount = obj.info[i].num
                    }
                   }*/
                  // var arr = JSON.stringify(aadd);
                  // console.log(aadd)
                  // var ad = {"startPayCount":"84","startPaySum":"466"}
                  // console.log(obj.info[4].num)
                   // console.log(aadd.toString()) 

                  // console.log(JSON.stringify(aadd));
                  android_obj.countFrequency('startLotPayCount');
                  // android_obj.calculateFrequency('startLotPaySum',obj.info[5].pay_money)
                  // android_obj.countFrequency(obj.info[4].num,startPayCount)
                 /* console.log(obj) 
                  var zf4 = obj.info[4].num
                  // console.log(zf4) 9
      ...
      var secData = { //发起支付金额
          'channel_id': '9',
          'type': 'pay_mon_zf',
          'pay_money':fo.total_pay_amount,
          'ip':returnCitySN["cip"]
        }
          console.log(obj.info.total_pay_amount)
      $.ajax({
        url:ConfigObj.localSite +'?m=cashier.deposit.number',
        data:secData,
        dataType:'json',
        ...
        success: function(obj){
          ...
          // alert(5)
              ...
              $.ajax({//次数接口
                ...
                success: function(obj){
                  /*for (var i = 0; i < obj.info.length; i++) {
                    android_obj.countFrequency(obj.info[i].type)
                    android_obj.calculateFrequency(obj.info[i].type,obj.info[i].pay_money)
                  }*/
                  /*var aadd = new Object();
                  for (var i = 0; i < obj.info.length; i++) {
                  ...
                  // console.log(aadd)
                  // var ad = {"startPayCount":"84","startPaySum":"466"}
                  // console.log(aadd.viewPayCount)
                   // console.log(aadd.toString()) 
                  // console.log(obj.info[5].pay_money)
                  // console.log(JSON.stringify(aadd));
                  // android_obj.calculateFrequency(obj.info[5].pay_money,startPaySum)
                  // android_obj.countFrequency('startPayCount');
                  // console.log(fo.total_pay_amount)
                  android_obj.calculateFrequency('startLotPaySum',fo.total_pay_amount)
                  /*console.log(obj) 
                  var zf5 = obj.info[5].num
                  var mon5 = obj.info[5].pay_money
                  // console.log(zf5 + mon5)
                  calculateFrequency(zf5,startPaySum,mon5) //支付金额*/
                    // android_obj.countFrequency(String startPayCount) 
                }
              });
        }
          ...
          // console.log(mony)
          // mony = obj.info.total_pay_amount
			    // 去掉遮罩层
...
//	  console.log(data)
    ...
    console.log(data.cash_amount)
        fMoney = data.cash_amount
          ...
          console.log(obj)
              ...
              $.ajax({//购彩金额次数接口
               ...
               /*   var aadd = new Object();
                  ...
                  // console.log(aadd)
                  // var ad = {"startPayCount":"84","startPaySum":"466"}
                  // console.log(obj.info[6].pay_money)
                   // console.log(aadd.toString()) 
                   // alert(7)
                  // console.log(JSON.stringify(aadd));
                  // android_obj.calculateFrequency("2",startLotPaySum)
                  // android_obj.countFrequency('startPayCount');
                  // console.log(fMoney)

                  // android_obj.calculateFrequency('startLotPaySum',fMoney)
                  /*console.log(obj) 
                  var zf6 = obj.info[6].num
                  var mon6 = obj.info[6].num
                  // console.log(zf6)
                  // console.log(mon6)
                  // calculateFrequency(String eventId,int money)
                  calculateFrequency(zf6,startLotPaySum,mon6) */
                  //calculateFrequency(String eventId,int money) 计算
                    // android_obj.countFrequency(String startPayCount) 
                }
              });
        }
